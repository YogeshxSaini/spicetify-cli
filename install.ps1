...
function Write-Log {
    ...
    for ($i = 0; $i -lt $Texts.Length -and $Texts.Length -eq $Emphasized.Length; $i++) {
        Write-Host -Object $Texts[$i] -NoNewline
        $logText += $Texts[$i]
    }
...
if ($PSVersionTable.PSVersion.Major -ge $PSMinVersion) {
    ...
    if ($isAdmin) {
        Write-Log -ActionText "WARNING" -Texts "The script was run as Administrator which isn't recommended`n" -Emphasized $false
    ...
    Write-Log -Texts "`nYour PowerShell version is less than ", "$PSMinVersion" -Emphasized $false, $true
